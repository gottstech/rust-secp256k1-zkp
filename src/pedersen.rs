...
pub struct RangeProof {
    ...
    #[serde(serialize_with = "u8_to_hex", deserialize_with = "hex_to_bp")]
...
impl Secp256k1 {
    ...
    fn commit_parse(
        &self,
        c_in: [u8; constants::PEDERSEN_COMMITMENT_SIZE],
    ) -> Result<CommitmentInternal, Error> {
        let c_out = unsafe {
            let mut c_out = CommitmentInternal::blank();
            ffi::secp256k1_pedersen_commitment_parse(self.ctx, c_out.as_mut_ptr(), c_in.as_ptr());
            c_out
        };
        Ok(c_out)
    }
    ...
    fn commit_ser(
        &self,
        c_in: [u8; constants::PEDERSEN_COMMITMENT_SIZE_INTERNAL],
    ...
    pub fn commit_i(&self, value: i64, blind: SecretKey) -> Result<Commitment, Error> {
        ...
        let mut commit_i = [0; constants::PEDERSEN_COMMITMENT_SIZE_INTERNAL];
        unsafe {
            ffi::secp256k1_pedersen_commit_i(
                self.ctx,
                commit_i.as_mut_ptr(),
                blind.as_ptr(),
                value,
...
mod tests {
    ...
    fn test_verify_commit_sum_random_keys() {
        ...
        let blind_sum = secp
            .blind_sum(vec![blind_pos.clone()], vec![blind_neg.clone()])
            .unwrap();
